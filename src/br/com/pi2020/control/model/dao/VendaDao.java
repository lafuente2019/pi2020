...
public class VendaDao {
	...
	public List<Venda> getVenda() throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			 vendas.add(new Venda(rs.getInt(1),rs.getString(2), rs.getFloat(3),rs.getInt(4), rs.getInt(5), rs.getString(6), rs.getString(7),rs.getFloat(8), rs.getInt(9)));
	...
	public void salvar(Venda venda) throws ClassNotFoundException, SQLException {
		...
		statement.setString(6, venda.getData());
	...
	public static List<Venda> buscar(String busca) throws SQLException, Exception {
		String sql = "SELECT p.idProduto, p.nome, p.preco, p.quantidade, v.idVenda, v.data, v.pagamento, v.subTotal,c.idCliente FROM Venda as v "
				+ " join cadastroProduto as p" 
				+ " on p.idProduto = v.idProduto"
				+ " join cadastroCliente as c"
				+ " on c.idCliente = v.idCliente"
				+ " join cadastroFuncionario as f"
		...
		try {
			...
			while (rs.next()) {
				...
				Venda V = new Venda(idProduto, nome, preco, quantidade, idVenda,data, pagamento, subTotal, idCliente);
				listaVenda.add(V);
			}
